...
let SMS = {
    ...
    send: function (input, callback) {
        let isFlash = input.type === 'flash' ? '-flash -text' : '-text';
        this._command(`gammu sendsms TEXT ${input.to} ${isFlash} "${input.text}"`, function(response){
            if (callback) callback(response);
        });
    },
    ...
    keepalive: function () {
        ...
        setInterval(
            () => {
                console.log('keepalive gammu with networkinfo');
                self._command(`gammu networkinfo`);
            },
